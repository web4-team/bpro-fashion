                ...
                @php $i=1; @endphp
                  ...
                  @foreach($data_sale as $row)
                    ...
                    <td>{{$i++}}</td>	
                      ...
                      <td>
                         <div class="btn-group">
                        <a href="{{ route('sales.edit', ['id' =>$row->id]) }}" class="btn btn-success btn btn-sm"><i class="fa fa-edit"></i></a>
                        <form action="{{ route('sale.destroy', $row->id)}}" method="post" onsubmit="return confirm('Are you sure?')">
                          @csrf
                          @method('DELETE')
