...
<div class="col-sm-12">
  ...
  <div class="float-right">
    <a style="margin: 19px;" href="{{ route('ab_items.create')}}" class="btn btn-primary">New Item</a>
  </div>     
      ...
      <tr>
        ...
        <td>
          <div class="btn-group">
              ...
              <a href="{{ route('ab_items.edit',$row->id)}}" class="btn btn-primary btn btn-sm"><i class="fas fa-edit"></i></a>
            <form action="{{ route('ab_items.destroy', $row->id)}}" method="post" onsubmit="return confirm('Are you sure?')">

              @csrf
              @method('DELETE')
              <button class="btn btn-danger btn btn-sm mx-1" type="submit"><i class="fa fa-trash"></i></button>
            </form>
             <a href="{{ route('ab_sales.show', $row->id) }}" data-toggle="tooltip" title="SaleList" class="btn btn-success btn btn-sm"><i class="fas fa-file"></i></a>
          </div>
        </td>
      </tr>
      @endforeach
