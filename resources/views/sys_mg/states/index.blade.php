            ...
            <tbody>
              ...
              @foreach($states as $state)
                    <tr>
                      ...
                      <td>
                      <a href="{{route('states.edit', $state->id)}}" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i></a>
                        
                        <form method="post" style="display: inline-block" action="{{route('states.destroy', $state->id)}}" onsubmit="return confirm('Are you sure?')">
                          @csrf
                          @method('DELETE')
